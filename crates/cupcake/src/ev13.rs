...
static TOTAL_BUTTON_DOWN_EVENTS: AtomicUsize = AtomicUsize::new(0);
static TOTAL_BUTTON_UP_EVENTS: AtomicUsize = AtomicUsize::new(0);
    ...
    P: GpioInput,
{
    fn scan(&mut self) {
        ...
        let event = match self.is_low {
            true => {
                TOTAL_BUTTON_DOWN_EVENTS.fetch_add(1, Ordering::SeqCst);
                BoardEvent::Button(self.button_id, ButtonState::Down)
            }
            false => {
                TOTAL_BUTTON_UP_EVENTS.fetch_add(1, Ordering::SeqCst);
                BoardEvent::Button(self.button_id, ButtonState::Up)
            }
        };
