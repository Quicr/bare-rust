...
use cmox::aead::{Aes128Ccm, Aes128Gcm, ChaCha20Poly1305};
use cmox::{cipher::*, initialize};
...
fn cbc_example(key: &[u8; 16], iv: &[u8; 16], plaintext: &[u8]) {
    let cipher = Aes128Cbc::new_with_key(key).expect("Failed to create AES-128 CBC cipher");
    ...
    let encrypted_len = cipher
        .encrypt(iv, &padded_plaintext[..total_len], &mut ciphertext)
        .expect("CBC encryption failed");

    // Decrypt
    let mut decrypted = [0u8; 64];
    let decrypted_len = cipher
        .decrypt(iv, &ciphertext[..encrypted_len], &mut decrypted)
...
fn cfb_example(key: &[u8; 16], iv: &[u8; 16], plaintext: &[u8]) {
    let cipher = Aes128Cfb::new_with_key(key).expect("Failed to create AES-128 CFB cipher");
    ...
    let mut ciphertext = [0u8; 64];
    let encrypted_len = cipher
        .encrypt(iv, &plaintext[..len], &mut ciphertext)
...
fn ctr_example(key: &[u8; 16], nonce: &[u8; 16], plaintext: &[u8]) {
    let cipher = Aes128Ctr::new_with_key(key).expect("Failed to create AES-128 CTR cipher");
    ...
    let mut ciphertext = [0u8; 64];
    let encrypted_len = cipher
        .encrypt(nonce, &plaintext[..len], &mut ciphertext)
        .expect("CTR encryption failed");

    // Decrypt (same operation as encrypt)
    let mut decrypted = [0u8; 64];
    let decrypted_len = cipher
        .decrypt(nonce, &ciphertext[..encrypted_len], &mut decrypted)
...
fn ofb_example(key: &[u8; 16], iv: &[u8; 16], plaintext: &[u8]) {
    let cipher = Aes128Ofb::new_with_key(key).expect("Failed to create AES-128 OFB cipher");
...
fn aes_gcm_example(key: &[u8; 16], nonce: &[u8; 12], aad: &[u8], plaintext: &[u8]) {
    let cipher = Aes128Gcm::new_with_key(key).expect("Failed to create AES-128-GCM cipher");
...
fn aes_ccm_example(key: &[u8; 16], nonce: &[u8; 12], aad: &[u8], plaintext: &[u8]) {
    let cipher = Aes128Ccm::new_with_key(key).expect("Failed to create AES-128-CCM cipher");
...
fn chacha20_poly1305_example(nonce: &[u8; 12], aad: &[u8], plaintext: &[u8]) {
    ...
    let cipher =
        ChaCha20Poly1305::new_with_key(&key).expect("Failed to create ChaCha20-Poly1305 cipher");
