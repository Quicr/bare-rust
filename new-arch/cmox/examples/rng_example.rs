...
use cmox::{initialize, rng::{CtrDrbg, CtrDrbgVariant}};
...
fn algorithm_variants_example() {
    ...
    let mut rng_128_small = CtrDrbg::new(
        CtrDrbgVariant::Aes128Small,
        &entropy_128,
        &nonce[..8],
        None,
    ...
    let mut rng_256_fast = CtrDrbg::new(
        CtrDrbgVariant::Aes256Fast,
        &entropy_256,
        &nonce,
        None,
    ...
    let mut rng_256_small = CtrDrbg::new(
        CtrDrbgVariant::Aes256Small,
        &entropy_256,
        &nonce,
        None,
    ...
    rng_128_fast.generate_bytes(&mut buffer).expect("128 fast generation failed");
    rng_128_small.generate_bytes(&mut buffer).expect("128 small generation failed");
    rng_256_fast.generate_bytes(&mut buffer).expect("256 fast generation failed");
    rng_256_small.generate_bytes(&mut buffer).expect("256 small generation failed");
}
...
fn reseeding_example() {
    ...
    let mut rng = CtrDrbg::new_default(&initial_entropy, &nonce)
        .expect("Failed to create DRBG");

    // Generate some initial random data
    let mut buffer = [0u8; 32];
    rng.generate_bytes(&mut buffer).expect("Initial generation failed");
    ...
    rng.generate_bytes(&mut buffer).expect("Post-reseed generation failed");
}
...
fn deterministic_testing_example() {
    ...
    let mut test_rng = CtrDrbg::new_deterministic_for_testing(12345)
        .expect("Failed to create test DRBG");

    // This will produce the same sequence every time for the same seed
    let mut test_output = [0u8; 16];
    test_rng.generate_bytes(&mut test_output)
